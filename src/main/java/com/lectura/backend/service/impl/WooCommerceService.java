...
import com.lectura.backend.service.*;
import com.lectura.backend.util.DateUtils;
...
public class WooCommerceService implements IWooCommerceService {
    ...
    public SimulateSaleResponse simulateSale(Long productId, Double price) throws Exception {
        transaction.begin();
        try {
            logger.info("Simulate sale: " + productId);
    ...
    public String registerSale(OrderDto order) throws Exception {
        Sale sale = Sale.findById(order.getOrderId());
        if (Objects.nonNull(sale) && Objects.nonNull(sale.getToken())) {
            return sale.getToken();
        }
        ...
        try {
            ...
            if (Objects.nonNull(publication) && !publication.getId().isEmpty()) {
                Price publicationPrice = publication.getPrice();
                try {
                    String customerId = order.getUsername().trim().replaceAll("[^a-zA-Z0-9]", "_");
                    ...
                    if (response.getStatus() == 201) {
                        var token = new TokenDescarga(LocalDateTime.now().plusMinutes(10));
                        sale = new Sale();
                        ...
                        sale.setToken(token.toString());

                        sale.persist();

                        var saleToken = new SaleToken();
                        saleToken.setSaleId(sale.getId());
                        saleToken.setToken(token.toString());
                        saleToken.setExpiration(token.getExpirationDateTime());
                        saleToken.persist();
    ...
    @Override
    @Transactional
    public String regenerateSaleToken(String orderId) throws Exception {
        Sale sale = Sale.findById(orderId);
        if (Objects.nonNull(sale) && !sale.getId().isEmpty()) {
            if (!sale.isDownloaded()) {
                var token = new TokenDescarga(LocalDateTime.now().plusDays(1));
                sale.setToken(token.toString());
                sale.persist();
    ...
    @Override
    public URI getDownloadUrl(String token, String uname) throws Exception {
        transaction.begin();
        try {
            var tokenDescarga = new TokenDescarga(token);
            ...
            if (saleOptional.isPresent() && !saleOptional.get().getId().isEmpty()) {
                ...
                if (tokenDescarga.getExpirationDateTime().isBefore(LocalDateTime.now())) {
    ...
    @Override
    public SaleStatusResponse getSaleStatus(String orderId) {
        Sale sale = Sale.findById(orderId);
        if (Objects.nonNull(sale) && !sale.getId().isEmpty()) {
            var response = new SaleStatusResponse();
            response.setCustomer(sale.getCustomer());
            response.setSku(sale.getSku());
            response.setFormat(sale.getFormat());
            response.setDateTime(sale.getDateTime());
            response.setOrderId(sale.getId());
            response.setDownloaded(sale.isDownloaded());
            return response;
        }
        return null;
    }
    ...
    public void synchronizeParameters() {
        try {
            synchronizeTags();
            var publications = repository.findToSynchronize(0, SIZE_PAGE_TO_PROCESS + 200, DateUtils.getMadridDate());
    ...
    private void emitMulti(MultiEmitter<? super Publication> emitter) {
        var publications = repository.findToSynchronize(0, SIZE_PAGE_TO_PROCESS, DateUtils.getMadridDate());
