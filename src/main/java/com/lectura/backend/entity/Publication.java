    ...
    public Price getPrice() throws Exception {
        if (prices.stream().anyMatch(p -> p.getCurrencyCode().equals("BOB"))) {
            ...
            if (list.size() == 1) {
                ...
                var priceFrom = list.stream().filter(p -> p.getRole().equals(Byte.valueOf("14"))
                                && DateUtils.getMadridDate() >= p.getDate())
                        .findFirst();
                if (priceFrom.isPresent()) {
                    return priceFrom.get();
                } else {
                    return list.stream().filter(p ->
                            (p.getRole().equals(Byte.valueOf("15")) && DateUtils.getMadridDate() <= p.getDate()) ||
